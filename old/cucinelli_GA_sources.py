...
def clean_and_convert(x):
    if isinstance(x, str):
        ...
        if '/' in x:
            parts = x.split('/')
            x = float(parts[0].strip()) / float(parts[1].strip())
    return x
...
df_new = process_csv("/Users/cristianmarchisio/Python/cucinelli_dataviz/GA_sources_new.csv", 'Sorgente/Mezzo', 'Sessioni')
df_old = process_csv("/Users/cristianmarchisio/Python/cucinelli_dataviz/GA_sources_old.csv", 'Sorgente/Mezzo', 'Sessioni')
...
df_diff_T = df_diff_T[df_diff_T.loc['Sessioni'].str.extract(r"(\d+)", expand=False).astype(float).sort_values(ascending=False).index]
...
df_diff_T.to_excel('Cucinelli_GA_sources_delta.xlsx')
